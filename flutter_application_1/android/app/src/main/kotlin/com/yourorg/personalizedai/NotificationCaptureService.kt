...
class NotificationCaptureService : NotificationListenerService() {
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification?) {
        ...
        if (shouldForwardToServer(this)) {
            Log.d(TAG, "Forwarding notification to backend (canonical model)")
            ContextApiClient.postCanonicalEvent(this, event.toString())
        } else {
    ...
    private fun shouldForwardToServer(context: Context): Boolean {
        ...
        // Default to local-only (no forwarding) unless user enables and upload_raw is true
        val forwarding = prefs.getBoolean(KEY_SERVER_FORWARDING_ENABLED, false)
        val uploadRaw = prefs.getBoolean(KEY_UPLOAD_RAW, false)
        return forwarding && uploadRaw
    }
    ...
    private fun initializeDefaultPreferences() {
        ...
        if (!prefs.contains(KEY_UPLOAD_RAW)) {
            editor.putBoolean(KEY_UPLOAD_RAW, false)
            Log.d(TAG, "Initialized upload_raw to false")
        }
    ...
    // Removed legacy forwardToBackendAsync; using ContextApiClient.postCanonicalEvent
    ...
    companion object {
        ...
        const val KEY_UPLOAD_RAW: String = "upload_raw"
    }
}