...
class NotificationForwarderService {
  ...
  static void _subscribeNative() {
    ...
    _nativeSub = _events.receiveBroadcastStream().listen((dynamic data) {
      try {
        ...
        // Ensure canonical minimal fields for backend compatibility
        if (map['app_name'] == null && map['package'] != null) {
          map['app_name'] = map['package'];
        }
        if (map['message'] == null && map['text'] != null) {
          map['message'] = map['text'];
        }
        if (map['title'] == null && map['sender'] != null) {
          map['title'] = map['sender'];
        }
  ...
  static void _pushEvent(Map<String, dynamic> map) {
    try {
      _eventCtrl.add(map);
      // Persist to Hive incoming_events
      try {
        // store minimal fields
        final box = LocalStorage.incomingEvents;
        final ts = DateTime.now().millisecondsSinceEpoch;
        box.put(map['event_id'] ?? 'evt-$ts', {
          'package': map['package'] ?? map['app_name'],
          'title': map['title'] ?? map['sender'],
          'text': map['text'] ?? map['message'],
          'timestamp': map['timestamp'] ?? ts,
        });
      } catch (_) {}
  ...
  static Future<bool> setUploadRaw(bool enabled) async {
    final res = await _methods.invokeMethod('setUploadRaw', {'enabled': enabled});
    return res == true;
  }
