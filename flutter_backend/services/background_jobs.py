...
class JobQueue:
    ...
    async def _handle_news_sync(self, job: BackgroundJob) -> Dict[str, Any]:
        ...
        try:
            ...
            # Fetch news articles (sync/async compatible)
            maybe_articles = connector.fetch_news(
                sources=sources, query=query
            )
            if asyncio.iscoroutine(maybe_articles):
                articles = await maybe_articles
            else:
    ...
    async def _handle_reddit_sync(self, job: BackgroundJob) -> Dict[str, Any]:
        ...
        try:
            ...
            # Fetch Reddit posts (sync/async compatible)
            maybe_posts = connector.fetch_posts(
                subreddits=subreddits, max_posts_per_subreddit=max_posts
            )
            if asyncio.iscoroutine(maybe_posts):
                posts = await maybe_posts
            else:
                posts = maybe_posts
    ...
    async def _handle_cleanup_old_data(self, job: BackgroundJob) -> Dict[str, Any]:
        ...
        try:
            ...
            # No delete API exposed; return count of would-be deletions
            candidates = store.feed_items.search(lambda d: d.get("created_at") and d.get("created_at") < cutoff_ts)
