...
"""
    ...
    def save_feed_items_with_embeddings(self, feed_items: List[FeedItem]) -> List[FeedItem]:
        """Persist feed items to NoSQL store and optionally create embeddings.

        Upserts by (user_id, origin_id, source) to avoid duplicates.
        Returns the input list for compatibility with callers/tests.
        """
        saved_items: List[FeedItem] = []
        try:
            for fi in feed_items:
                # Duplicate check in NoSQL store
                exists = store.feed_items.search(
                    lambda d: d.get("origin_id") == getattr(fi, "origin_id", None)
                    and d.get("user_id") == getattr(fi, "user_id", None)
                    and d.get("source") == getattr(fi, "source", None)
                )
                if exists:
                    logger.info(f"Duplicate WhatsApp feed item skipped: {getattr(fi, 'origin_id', None)}")
                    continue

                # Convert FeedItem to document
                doc = {
                    "user_id": getattr(fi, "user_id", None),
                    "origin_id": getattr(fi, "origin_id", None),
                    "source": getattr(fi, "source", None),
                    "title": getattr(fi, "title", None),
                    "summary": getattr(fi, "summary", None),
                    "text": getattr(fi, "content", None),
                    "date": getattr(fi, "date", None).isoformat() if getattr(fi, "date", None) else None,
                    "priority": int(getattr(fi, "priority", 0.5) * 10) if getattr(fi, "priority", None) else 5,
                    "relevance_score": getattr(fi, "relevance", None),
                    "entities": getattr(fi, "entities", None),
                    "has_tasks": bool(getattr(fi, "meta_data", {}).get("extracted_tasks")) if getattr(fi, "meta_data", None) else None,
                    "meta_data": getattr(fi, "meta_data", None),
                    "created_at": __import__("time").time(),
                    "updated_at": __import__("time").time(),
                }
                store.insert("feed_items", doc)

                # Optional: create embedding using available pipeline and vector store
                try:
                    embedding = self.embeddings_pipeline.embed_text(getattr(fi, "content", "") or "")
                    if embedding:
                        # If you have a vector store that uses NoSQL, integrate here; otherwise skip
                        pass
                except Exception as e:
                    logger.error(f"Error creating embedding for WhatsApp feed item {doc.get('origin_id')}: {e}")

                saved_items.append(fi)
        except Exception as e:
