...
class GmailConnector:
    ...
    def handle_oauth_callback(self, authorization_code: str, state: str) -> bool:
        ...
        try:
            ...
            # Store credentials in NoSQL connectors
            record = {
                "user_id": user_id,
                ...
                "token_expires_at": credentials.expiry.isoformat() if getattr(credentials, "expiry", None) else None,
                "config_data": {
                    "email": credentials.id_token.get("email") if getattr(credentials, "id_token", None) else None,
                    "name": credentials.id_token.get("name") if getattr(credentials, "id_token", None) else None,
                },
    ...
    def get_credentials(self, user_id: int) -> Optional[Credentials]:
        ...
        try:
            # Read connector from NoSQL store
            configs = store.connectors.search(lambda c: c.get("user_id") == user_id and c.get("connector_type") == "gmail" and c.get("is_enabled") is True)
            cfg = configs[0] if configs else None
            if not cfg or not cfg.get("access_token"):
                return None
            # Build Credentials only if Google APIs available
            if not GOOGLE_APIS_AVAILABLE:
                return None
            credentials = Credentials(
                token=self._decrypt_token(cfg.get("access_token", "")),
                refresh_token=self._decrypt_token(cfg.get("refresh_token", "")) if cfg.get("refresh_token") else None,
                token_uri="https://oauth2.googleapis.com/token",
                client_id=self.client_id,
                client_secret=self.client_secret,
                scopes=self.scopes
            ...
            if credentials.expired and credentials.refresh_token:
                ...
                cfg["access_token"] = self._encrypt_token(credentials.token)
                cfg["token_expires_at"] = credentials.expiry.isoformat() if getattr(credentials, "expiry", None) else None
